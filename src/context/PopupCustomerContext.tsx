...
const PopupCustomerContext = createContext<PopupContextType | undefined>(undefined);

export function PopupCustomerContextProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <PopupCustomerContext.Provider value={{ showPopup,closePopup  }}>
      {children}
    ...
    </PopupCustomerContext.Provider>
  );
}

export function usePopupMessenger() {
  const context = useContext(PopupCustomerContext);
