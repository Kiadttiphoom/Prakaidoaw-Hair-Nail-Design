...
import { Scissors, Mail, Lock, AlertCircle, Eye, EyeOff, CheckCircle, AlertTriangle, X } from "lucide-react";
import {NotificationType,Notification} from "@/../type";
import NotificationUI, { NotificationRef } from "@/components/UI/Notification/NotificationUI";
...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (userid.trim() === "") {
      notificationRef.current?.showNotification('warning', 'กรุณากรอกชื่อผู้ใช้')
      setIsVisible(true);
      return;
    } else if (password.trim() === "") {
      notificationRef.current?.showNotification('warning', 'กรุณากรอกรหัสผ่าน')
      setIsVisible(true);
      return;
    }
    ...
    try {
      ...
      if (res.status === 200) {
        notificationRef.current?.showNotification('success', 'เข้าสู่ระบบสำเร็จ')
        setIsVisible(true);
        setTimeout(() => {
          setIsVisible(false);
        ...
        notificationRef.current?.showNotification('error', data.message)
        setIsVisible(true);
        return;
      }
    } catch (err) {
      notificationRef.current?.showNotification('error', "เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง")
      setIsVisible(true);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <>
      <NotificationUI ref={notificationRef} />
