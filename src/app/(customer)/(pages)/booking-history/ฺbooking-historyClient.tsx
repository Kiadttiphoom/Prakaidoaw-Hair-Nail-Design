...
export default function BookingHistoryClient({ bookinghistory }: Props) {
  ...
  {bookinghistory.map((booking, index) => {
    ...
    //const paymentInfo = getPaymentStatus(booking.payment_status);

    return (
      <div
        key={index}
        className="bg-white rounded-2xl border border-gray-200/60 p-5 shadow-sm hover:shadow-md transition-all duration-300"
      >
        {/* Header with Status */}
        <div className="flex items-start justify-between mb-4 pb-4 border-b border-gray-200/60">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-gray-400 tracking-wider uppercase mb-1">
              รหัสการจอง
            </p>
            <p className="text-sm text-gray-900 font-light tracking-wide truncate">
              {booking.booking_code}
            </p>
          </div>
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${statusConfig.border} ${statusConfig.bg} ml-2 flex-shrink-0`}>
        ...
        {/* Service Title */}
        <div className="mb-4">
          <h3 className="text-lg text-gray-900 font-light mb-1 line-clamp-1">
        ...
        <div className="space-y-3 mb-4">
          ...
          <div className="flex items-start gap-2">
            <User className="w-4 h-4 text-gray-500 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] text-gray-400 mb-0.5">ชื่อผู้จอง</p>
          ...
          {/* เบอร์โทรศัพท์ */}
          <div className="flex items-start gap-2">
            <Phone className="w-4 h-4 text-gray-500 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] text-gray-400 mb-0.5">เบอร์โทรศัพท์</p>
              <p className="text-sm text-gray-900 font-light truncate">
          ...
          <div className="flex items-start gap-2">
            <Calendar className="w-4 h-4 text-gray-500 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] text-gray-400 mb-0.5">วันที่และเวลา</p>
        ...
        {/* Note - Compact */}
        {booking.note && (
          <div className="mb-4 p-3 rounded-xl bg-gray-50 border border-gray-200/60">
            <div className="flex items-start gap-2">
              <FileText className="w-4 h-4 text-gray-500 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
              <div className="min-w-0 flex-1">
                <p className="text-[10px] text-gray-400 mb-0.5">หมายเหตุ</p>
                <p className="text-xs text-gray-700 font-light leading-relaxed line-clamp-2">
                  {booking.note}
                </p>
              </div>
            </div>
        ...
        <div className="pt-4 border-t border-gray-200/60">
          <div className="flex items-baseline gap-1">
            <span className="text-xs text-gray-500 font-light">ราคา</span>
